...
class FileFinder
  constructor: (@app) ->
    # Rebuild the index whenever the app's path changes.
    @app.workspace.on 'kitana.workspace.path_changed', @rebuild_index

  rebuild_index: =>
    new_index = []
    @_index_directory @app.workspace.path, new_index
    @index = new_index
  ...
  _index_directory: (directory_path, new_index) ->
    ...
    for entry in entries
      ...
      if fs.lstatSync(full_entry_path).isDirectory()
        ...
        new_index.push full_entry_path[@app.workspace.path.length+1..]
